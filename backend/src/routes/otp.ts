...
import { issueStepUpOtp, verifyStepUpOtp } from "../security/otp.service";
...
otpRouter.post("/verify", async (req, res) => {
  try {
    ...
    const result = await verifyStepUpOtp(stepUp.userId, code);
...
otpRouter.post("/resend", async (req, res) => {
  try {
    ...
    const { otp, expiresAt } = await issueStepUpOtp(stepUp.userId);
