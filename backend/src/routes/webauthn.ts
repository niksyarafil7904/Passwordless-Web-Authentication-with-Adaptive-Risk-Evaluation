...
import { asBuffer, pickFirst, toPrismaBytes, toUint8Array } from "../webauthn/helpers";
import { issueStepUpOtp } from "../security/otp.service";
import { sendStepUpOtpEmail } from "../security/email.service";
import { scoreLoginRisk, type RiskResult } from "../risk/scoreLoginRisk";

import { logLoginAttempt } from "../audit/loginAttempt";

export const webauthnRouter = Router();

const { rpID, rpName, expectedOrigin } = webauthnConfig;

// Helper function to parse User Agent for device info
function parseUserAgent(ua: string | null) {
  if (!ua) return { os: "Unknown", browser: "Unknown", deviceName: "Unknown Device" };
...
function getUserAgent(req: any) {
  return (req.get("user-agent") as string | undefined) ?? null;
}
...
webauthnRouter.post("/register/verify", async (req, res) => {
  try {
    ...
    console.log("VERIFY body.id:", (body as any)?.id);
    ...
    const credentialIdRaw = pickFirst(
      reg.credentialID,
      reg.credentialId,
      reg.id,
      reg.credential?.id,
      reg.credential?.credentialID,
      reg.credential?.credentialId,
    );

    const publicKeyRaw = pickFirst(
      reg.credentialPublicKey,
      reg.publicKey,
      reg.credential?.publicKey,
      reg.credential?.credentialPublicKey,
    );

    const counter = pickFirst<number>(reg.counter, reg.credential?.counter, 0) ?? 0;
    ...
    const credentialIdBuf = asBuffer(credentialIdRaw, "credentialID");
    const publicKeyBuf = asBuffer(publicKeyRaw, "credentialPublicKey");
    ...
    const transports =
      ((body as any)?.response?.transports as string[] | undefined) ??
    ...
    await prisma.webAuthnCredential.create({
      data: {
        ...
        credentialId: toPrismaBytes(credentialIdBuf),
        publicKey: toPrismaBytes(publicKeyBuf),
        counter,
        ...(transports ? { transports: transports as any } : {}),
...
webauthnRouter.post("/login/verify", async (req, res) => {
  try {
    ...
    const credentialIdB64 = (body as any)?.id as string | undefined;
    if (!credentialIdB64) {
      if (pendingUserId) {
        await logLoginAttempt({
          success: false,
    ...
    const matched = creds.find(
      (c) => Buffer.from(c.credentialId).toString("base64url") === credentialIdB64,
    );
    ...
    const newCounter =
      (pickFirst<number>(info?.newCounter, info?.counter, matched.counter) ?? matched.counter);
    ...
    try {
      const geoResult = await geoFromIp(ip);
    ...
    try {
      const riskPromise = scoreLoginRisk({
      ...
      risk = await Promise.race([riskPromise, timeoutPromise]) as RiskResult;
    ...
    if (risk.riskLevel === "high") {
      ...
      const { otp, expiresAt } = await issueStepUpOtp(matched.userId);
